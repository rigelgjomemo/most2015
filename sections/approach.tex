...
\todo{W1.1 We moved the example about $V_E$ and $V_I$ (in orange text) from the following paragraph to this paragraph, since $V_E$ and $V_I$ are introduced here.}
...
{\color{orange} We highlight at this point that $V_E$ is manually determined by an attacker depending on the type of attack s(he) plans to carry out and on the particular sink in the application. For instance, if the the attacker decides to target UI components for a phishing attack, $V_E$ will contain variables whose values determine the look and feel of the application. If, on the other hand, the attacker decides to target a network operation that sends some data to a server with the goal of carrying out a cross-site scripting attack on the server, the state $V_E$ will contain the variables being sent to the server with values containing cross-site scripting code.

Therefore, in our approach, we take the state $V_E$ as input and try to automatically determine if one or more related states $V_I$ exist.} Once $V_I$ is determined, it then serves as a proof of vulnerability for $V_E$. To answer these questions, the relationship between the application state at any point in the program and the application state $V_I$ at the source statements must be made explicit. The discovery of such relationship and its modeling as a function $F$, such that we can automatically compute $V_I$ as $V_I = F(V_E)$ is at the core of our approach. 
\todo{W1.1. We removed the bullet points that were in the previous version and rewrote the text, mentioning that $V_E$ is manually created, that it depends on the goal of the attacker and on the sink. Given $V_E$ in a point, we want to automatically determine $V_I$ at the source (orange text).}
% We highlight at this point that depending on the operations along the execution paths there may exist several states $V_I$, which induce the same state $V_E$. For instance, if an attacker aims at inducing the state $V_E = \{ url, ``http://malicious.example.com''\}$ in line 19 of Figure \ref{lst:example}, there can be many possible states $V_I$ after the \textit{source} statement at line 5, having that value for the variable $url$ and different values for the variables $path$ and user. Our approach discovers all these possible states. 
